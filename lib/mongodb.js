...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    if (prop && prop.type === Buffer) {
      ...
      } else if (prop && prop.mongodb && prop.mongodb.dataType === 'ObjectId') {
        data[p] = data[p] ? String(data[p]) : data[p]
      }
    } else if (
      data[p] &&
      prop &&
...
MongoDB.prototype.toDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    if (isGeoPoint && this.settings.enableGeoIndexing === true) {
      data[p] = {
        coordinates: [data[p].lng, data[p].lat],
        type: 'Point',
      };
    } else if (data[p] && prop && prop.mongodb && prop.mongodb.dataType === 'ObjectId') {
      data[p] = data[p] ? ObjectID(data[p]) : data[p]
    }
  }
...
MongoDB.prototype.coerceId = function (model, id, options) {
  ...
  if (idProp && typeof idProp.type === 'function') {
    ...
    // Esta comprobación no es correcta por algún motivo y los id que gestiona
    //     loopback (string) no se acaban conviertiendo en ObjectIds (los que
    //     maneja mongo por defecto). Por lo tanto todas las propiedades con
    //     "id" : true se van a convertir a ObjectId. AVISO: ESTO NO PERMITIRA
    //     CLAVES IDS DE DOCUMENTOS QUE NO SEAN OBJECTID
    //     se añade comprobacion idProp.id
    if (idProp.id || self.isObjectIDProperty(model, idProp, idValue, options)) {
  ...
  Object.keys(where).forEach(function (k) {
    ...
    if (spec) {
      if (spec === 'between') {
        ...
        query[k] = {
          $in: cond.map(function (x) {
            if (self.isObjectIDProperty(model, prop, x, options, forceCoercion))
              return ObjectID(x);
            return x;
          }),
        };
      ...
      if (cond === null) {
        ...
        if (self.isObjectIDProperty(model, prop, cond, options, forceCoercion)) {
          cond = ObjectID(cond);
        }
        query[k] = cond;
      }
 ...
 *
 */
MongoDB.prototype.isObjectIDProperty = function (model, prop, value, options, forceCoercion) {
  if (
    prop &&
    (prop.type === ObjectID ||
    ...
    if (strict && !forceCoercion) return false; // unless explicitly typed, don't coerce
